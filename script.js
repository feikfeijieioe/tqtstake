(() => {
  ...
  const replaceARS = () => {
    ...
    const selectors = [
      ...
      'span[data-role="footer-balance"]',
      'span[data-role="balance-label-value"]',
      'span[data-role="total-bet-label-value"]',
      'div.tableLimits--97b4b'
    ];
    document.querySelectorAll(selectors.join(',')).forEach(el => {
      if (!shouldSkip(el, elements) && el.textContent.includes('ARS')) {
        el.textContent = el.textContent.replace(/ARS[\s\u00A0]*\$/g, '$');
        console.log(`Replaced ARS with $ in "${el.textContent}"`);
      }
    });
