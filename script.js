(() => {
  ...
  const WITHDRAWAL_SELECTOR = 'div.currency.svelte-13xyujb span.weight-normal.line-height-default.align-left.size-default.text-size-default.variant-highlighted.numeric.with-icon-space.is-truncate.svelte-1f6lug3';
  ...
  const replaceWithdrawalAmount = () => {
    document.querySelectorAll(WITHDRAWAL_SELECTOR).forEach(span => {
      const parent = span.closest('div.flex.flex-row.justify-between.gap-2');
      if (parent) {
        const labelSpan = parent.querySelector('span.weight-normal.line-height-120pct.align-left.size-default.text-size-default.variant-subtle.with-icon-space.svelte-1f6lug3');
        if (labelSpan?.textContent.trim() === 'Minimum Withdrawal') {
          span.textContent = '$3';
        }
      }
    });
      ...
      muts.forEach(m => {
        ...
        m.addedNodes.forEach(n => {
          if (n.nodeType === 1) {
            ...
            n.querySelectorAll?.(WITHDRAWAL_SELECTOR).forEach(span => {
              replaceWithdrawalAmount();
            });
