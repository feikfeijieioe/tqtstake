(() => {
  ...
  // Nouvelle fonction pour remplacer la couleur de la bordure
  const replaceBorder = () => {
    const elements = document.querySelectorAll('div.flex.flex-col.justify-center.rounded-lg.w-full.bg-grey-700');
    elements.forEach(el => {
      if (el.style.border.includes('#2f4553')) {
        el.style.border = '2px solid #6fdde7';
      }
    });
  };
  ...
  const setupTextObserver = () => {
    const observer = new MutationObserver(muts => {
      ...
      replaceBorder(); // Appliquer le remplacement de bordure lors des mutations
    });
  ...
  (async () => {
    ...
    replacePaths();
    replaceBorder(); // Appeler la fonction pour remplacer la bordure initialement
    setupTextObserver();
    setupDecimalLogger();
    setInterval(fetchPrices, 60000);
    setInterval(() => { convertAll(); replaceARS(); replaceNoneAndBronze(); replaceBorder(); }, 1000); // Ajouter replaceBorder dans l'intervalle
    new MutationObserver(muts => {
      ...
      replaceNoneAndBronze();
      replacePaths();
      replaceBorder(); // Appliquer le remplacement de bordure pour les nouveaux éléments
