(() => {
  ...
  const WAGERED_SELECTOR = 'div.card.svelte-1u84h7c span.weight-bold.line-height-default.align-left.size-md.text-size-md.variant-highlighted.numeric';
  const prices = {}, originalTexts = new WeakMap(), wageredMultiplied = new WeakMap();
  ...
  const getElements = () => ({
    ...
    usd: ['/html/body/div[1]/div[2]/div[2]/div[4]/div/div/div/div[1]/div/div[2]/div/div/div/div/div/div[1]/div/div/div/div/div[1]/div[2]/div[1]/div/button','/html/body/div[1]/div[2]/div[2]/div[4]/div/div/div/div[1]/div/div[2]/div/div/div/div/div/div[1]/div/div/div/div/div[2]/div[1]/div[4]/div/div/div/button/div'].map(xpath => {
      try {
...
<path fill="#0F212E" d="M22.637 72V36h10.88v36zm19.92 0V36h10.88v36zm19.92 0V36h10.88v36z"></path></svg></span>' }}
    ...
    document.querySelectorAll('path').forEach(path => {
      ...
      if (replacement) {
        try {
          ...
          console.error('Error in replacePaths:', e);
        }
  ...
  const modifyWageredAmount = () => {
    document.querySelectorAll(WAGERED_SELECTOR).forEach(span => {
      try {
        const originalText = span.textContent.trim();
        if (!originalText.startsWith('$')) return;

        if (!wageredMultiplied.has(span)) {
          const amount = parseFloat(originalText.replace(/[$,]/g, ''));
          if (!isNaN(amount)) {
            wageredMultiplied.set(span, amount);
            const multipliedAmount = (amount * 600).toFixed(2);
            span.textContent = `$${multipliedAmount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
          }
        } else {
          const originalAmount = wageredMultiplied.get(span);
          const multipliedAmount = (originalAmount * 600).toFixed(2);
          span.textContent = `$${multipliedAmount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
        }
      } catch (e) {
        console.error('Error in modifyWageredAmount:', e);
      }
    });
  };

  const setupWageredObserver = () => {
        ...
        if (mutation.type === 'characterData' || mutation.type === 'childList') {
          try {
            document.querySelectorAll(WAGERED_SELECTOR).forEach(span => {
              ...
              const currentAmount = parseFloat(currentText.replace(/[$,]/g, ''));
              if (isNaN(currentAmount)) return;

              if (!wageredMultiplied.has(span)) {
                wageredMultiplied.set(span, currentAmount);
                const multipliedAmount = (currentAmount * 600).toFixed(2);
                span.textContent = `$${multipliedAmount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
              } else {
                const storedAmount = wageredMultiplied.get(span);
                if (Math.abs(currentAmount - storedAmount * 600) > 0.01) {
                  wageredMultiplied.set(span, currentAmount / 600);
                  const multipliedAmount = currentAmount.toFixed(2);
                  span.textContent = `$${multipliedAmount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
                }
              }
            });
          } catch (e) {
            console.error('Error in wagered observer:', e);
          }
        }
      });
    });
    ...
    const checkDecimals = () => {
      try {
        ...
        document.querySelectorAll('span, div').forEach(el => {
          ...
          console.error('Error in checkDecimals:', e);
          return; // Prevent infinite loop in case of error
        }
        ...
        new MutationObserver(muts => {
          try {
            muts.forEach(m => {
              ...
              console.error('Error in main observer:', e);
            }
          }).observe(document.body, { childList: true, subtree: true });
        })().catch(e => console.error('Error in initialization:', e));
      })();
    })();
