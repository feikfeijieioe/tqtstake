(() => {
  ...
  const multiplyLTC = () => {
    ...
    if (!input || input.dataset.betAmountActiveCurrency !== 'ltc') {
      return;
    }
  ...
  const waitForInputAndLTC = () => {
    const check = () => {
      const input = document.querySelector('input[data-testid="input-game-amount"]');
      const ltcElements = document.querySelectorAll(CONV_SELECTOR);
      if (input && ltcElements.length > 0 && input.dataset.betAmountActiveCurrency === 'ltc') {
        multiplyLTC();
      } else {
    ...
    const observer = new MutationObserver(muts => {
      ...
      muts.forEach(m => {
        ...
        m.addedNodes.forEach(n => {
          if (n.nodeType === 1) {
            if (n.matches?.('input[data-testid="input-game-amount"]')) {
              hookInput(n);
            }
            n.querySelectorAll?.('input[data-testid="input-game-amount"]').forEach(hookInput);
