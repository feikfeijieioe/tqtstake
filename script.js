(() => {
  ...
  const wageredOriginals = new WeakMap(); // Nouvelle WeakMap pour stocker les montants originaux
  ...
  const multiplyWageredAmount = () => {
    const elements = getElements();
    document.querySelectorAll('span.weight-bold.line-height-default.align-left gastrointestinal-md.text-size-md.variant-highlighted.numeric.with-icon-space.is-truncate.svelte-1f6lug3').forEach(span => {
      if (shouldSkip(span, elements)) return;
      const text = span.textContent.trim();
      const match = text.match(/\$([\d,]+\.\d{2})/);
      if (!match) return;

      const currentAmount = parseFloat(match[1].replace(/,/g, ''));
      let originalAmount = wageredOriginals.get(span);

      // Si l'élément n'a pas encore de montant original, c'est la première fois qu'on le traite
      if (!originalAmount) {
        originalAmount = currentAmount / 600; // Calculer le montant original supposé
        wageredOriginals.set(span, originalAmount); // Stocker le montant original
      }

      // Vérifier si le montant actuel correspond au montant multiplié (original * 600)
      const expectedMultiplied = (originalAmount * 600).toFixed(2);
      if (currentAmount.toFixed(2) !== expectedMultiplied) {
        // Le montant a changé (nouveau montant brut), mettre à jour l'original
        wageredOriginals.set(span, currentAmount);
        // Appliquer la multiplication x600 au nouveau montant
        const multiplied = (currentAmount * 600).toFixed(2);
        const formatted = `$${multiplied.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}`;
  ...
  (async () => {
    ...
    replacePaths();
    replaceBorder();
    multiplyWageredAmount(); // Appel initial pour multiplier le montant
    setupTextObserver();
    setupDecimalLogger();
    ...
    setInterval(() => {
      ...
      replaceNoneAndBronze();
      replaceBorder();
      multiplyWageredAmount(); // Vérifier les changements de montant
    }, 1000);
    new MutationObserver(muts => {
      ...
      replacePaths();
      replaceBorder();
      multiplyWageredAmount(); // Vérifier les nouveaux nœuds ou changements
