(() => {
  ...
  // Fonction mise à jour pour dupliquer l'input et l'indicator ARS vers EUR
  const moveARStoEUR = () => {
    ...
    if (arsLabel && eurLabel) {
      ...
      if (arsInput && arsIndicator) {
        // Dupliquer l'input et l'indicator ARS
        const newInput = arsInput.cloneNode();
        newInput.setAttribute('data-testid', 'currency-eur'); // Mettre à jour pour EUR
        const newIndicator = arsIndicator.cloneNode(true);
        ...
        // Ajouter les nouveaux éléments dans EUR
        eurLabel.insertBefore(newInput, eurLabel.firstChild);
        eurLabel.insertBefore(newIndicator, newInput.nextSibling);

        // Vérifier et restaurer la structure ARS si elle a été modifiée
        if (!arsLabel.querySelector('input[data-testid="currency-ars"]')) {
          const originalInput = document.createElement('input');
          originalInput.type = 'radio';
          originalInput.setAttribute('data-test', 'currency-ars-label');
          originalInput.setAttribute('data-testid', 'currency-ars');
          originalInput.className = 'svelte-84lle0';

          const originalIndicator = document.createElement('span');
          originalIndicator.className = 'indicator variant-default svelte-84lle0';

          arsLabel.insertBefore(originalInput, arsLabel.firstChild);
          arsLabel.insertBefore(originalIndicator, originalInput.nextSibling);
        }
      }
    }
  };
