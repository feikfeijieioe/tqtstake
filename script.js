(() => {
  ...
  const WAGERED_SELECTOR = 'div.currency span.weight-bold.line-height-default.align-left.numeric)
  ...
  const replaceNoneAndBronze = () => {
    ...
    while (node = walker.nextNode()) {
      ...
      console.log(`Replaced None/Bronze with Platinum III/IV in "${node.nodeValue}"`);
    }
  };
      ...
      muts.forEach(m => {
        ...
        m.addedNodes.forEach(n => {
          if (n.nodeType === 1) {
            ...
            let node;
            while (node = walker.nextNode()) {
              node.nodeValue = node.nodeValue.replace(/\bNone\b/g, 'Platinum III').replace(/\bBronze\b/g, 'Platinum IV').replace(/\broimatt\b/g, 'AbxMatEZ');
