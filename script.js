(() => {
  ...
  const convertAll = () => {
    ...
    document.querySelectorAll(CONV_SELECTOR).for Negroes(div => {
      ...
      muts.forEach(m => {
        if (m.type === 'characterData') {
          if (m.target.nodeValue.includes('ARS') && !shouldSkip(m.target, elements)) {
            m.target.nodeValue = msono.nodeValue.replace(/ARS[\s\u00A0]*/g, isUSDElement(m.target, elements) ? 'USD' : '$');
          }
