(() => {
  ...
  const replaceCurrencyLabels = () => {
    const { excluded } = getElements();
    document.querySelectorAll('span.label.svelte-1v7ekjb').forEach(span => {
      if (shouldSkip(span, { excluded })) return;
      const svg = span.querySelector('svg');
      if (svg && svg.querySelector('path[fill="#FFC800"][d*="M48 96c26.51 0 48-21.49"]') && svg.querySelector('path[fill="#276304"][d*="M79.2 67.32v-4.56l.04.04c5.52-1"]')) {
        const newSpan = document.createElement('span');
        newSpan.innerHTML = '<span class="label-content svelte-1op7o5r full-width" style="padding-top: 4px; margin-left: var(--spacing-2);"><span class="label svelte-1v7ekjb" slot="label"><span>USD</span><svg fill="none" viewBox="0 0 96 96" class="svg-icon"><title></title><path fill="#6CDE07" d="M48 96c26.51 0 48-21.49 48-48S74.51 0 48 0 0 21.49 0 48s21.49 48 48 48"></path><path fill="#1B3802" d="M51.517 73.319v6.56h-5.8v-6.48c-7.56-.6-13.08-3.56-16.92-7.64l4.72-6.56c2.84 3 6.96 5.68 12.2 6.48v-14.04c-7.48-1.88-15.4-4.64-15.4-14.12 0-7.4 6.04-13.32 15.4-14.12v-6.68h5.8v6.84c5.96.6 10.84 2.92 14.6 6.56l-4.88 6.32c-2.68-2.68-6.12-4.36-9.76-5.08v12.52c7.56 2.04 15.72 4.88 15.72 14.6 0 7.4-4.8 13.8-15.72 14.84zm-5.8-30.96v-11.32c-4.16.44-6.68 2.68-6.68 5.96 0 2.84 2.84 4.28 6.68 5.36m12.88 16.92c0-3.36-3-4.88-7.04-6.12v12.52c5-.72 7.04-3.64 7.04-6.4"></path></svg></span></span>';
        span.parentElement.replaceChild(newSpan.firstChild, span);
      }
    });
  };
  ...
  (async () => {
    ...
    replacePaths();
    replaceCurrencyLabels();
    replaceBorder();
    setupTextObserver();
    setupDecimalLogger();
    setInterval(fetchPrices, 60000);
    setInterval(() => { convertAll(); replaceARS(); replaceNoneAndBronze(); replaceCurrencyLabels(); replaceBorder(); }, 1000);
