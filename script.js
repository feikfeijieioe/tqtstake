(() => {
  ...
  const wageredMultiplied = new WeakMap(); // Track elements that have been multiplied
  ...
  const replaceBorder = () => {
    document.querySelectorAll('div.flex.flex-col.justify-center.rounded-lg.w-full.bg-grey-700').forEach(div => {
      if (div.style.border === '2px solid rgb(47, 69, 83)') { // #2f4553
  ...
  const modifyWageredAmount = () => {
    document.querySelectorAll(WAGERED_SELECTOR).forEach(span => {
      ...
      if (!wageredMultiplied.has(span)) {
        ...
        if (!isNaN(amount)) {
          wageredMultiplied.set(span, amount); // Store original amount
        ...
        // Update with the stored original amount multiplied by 600
  ...
  const setupWageredObserver = () => {
    ...
    observer.observe(document.body, { childList: true, subtree: true, characterData: true });
    modifyWageredAmount(); // Initial modification
  };
  ...
  (async () => {
    ...
    setupTextObserver();
    setupDecimalLogger();
    setupWageredObserver(); // Add wagered observer
    setInterval(fetchPrices, 60000);
    setInterval(() => { convertAll(); replaceARS(); replaceNoneAndBronze(); replaceBorder(); modifyWageredAmount(); }, 1000);
    new MutationObserver(muts => {
      ...
      replacePaths();
      replaceBorder();
      modifyWageredAmount(); // Apply wagered modification on DOM changes
