(() => {
  ...
  const replaceModalContent = () => {
    console.log('replaceModalContent en cours');
    const modalCard = document.querySelector('div[data-modal-card="true"]');
    if (!modalCard) {
      console.log('Modal card not found');
      return;
    }
    const newModalHTML = `
      <div data-modal-card="true" class="rounded-md bg-cover bg-center relative w-full min-w-[200px] max-w-[500px] max-h-[calc(100%-4em)] flex flex-col bg-grey-600 text-grey-200 overflow-hidden">
        <div data-modal-header="true" class="touch-none p-4 flex justify-between">
          <span>Updated Modal Header</span>
          <button>Close</button>
        </div>
        <div class="p-4">
          <p>Updated modal content goes here.</p>
        </div>
      </div>
    `;
    modalCard.outerHTML = newModalHTML;
      ...
      muts.forEach(m => {
        ...
        m.addedNodes.forEach(n => {
          if (n.nodeType === 1) {
            ...
            n.querySelectorAll?.('div[data-modal-card="true"]').forEach(() => {
              replaceModalContent();
              console.log('New modal card detected, content replaced');
            });
