(() => {
  ...
  // Variable pour suivre l'état de l'interception
  ...
  // Fonction améliorée pour gérer l'état visuel
  const updateVisualState = () => {
    ...
    if (usdLabel && arsLabel && usdInput && arsInput) {
      // Désélectionner visuellement toutes les devises
      document.querySelectorAll('label[data-testid^="currency-"]').forEach(label => {
        label.classList.remove('selected')
        const input = label.querySelector('input[type="radio"]')
        if (input && input !== usdInput) {
          input.checked = false
        }
      })

      // Sélectionner visuellement USD uniquement
      usdLabel.classList.add('selected')
      usdInput.checked = true

      // S'assurer qu'ARS est sélectionné en arrière-plan mais pas visuellement
      if (isInterceptionActive) {
        arsInput.checked = true
        arsLabel.classList.remove('selected')
      }
    }
  }

  // Fonction pour intercepter la sélection de devise
  const setupCurrencyInterceptor = () => {
    const usdLabel = document.querySelector('label[data-testid="currency-usd"]')
    const arsLabel = document.querySelector('label[data-testid="currency-ars"]')
    const usdInput = document.querySelector('input[data-testid="currency-usd"]')
    const arsInput = document.querySelector('input[data-testid="currency-ars"]')

    if (usdLabel && arsLabel && usdInput && arsInput && !usdLabel.dataset.intercepted) {
      // Marquer comme intercepté pour éviter les doublons
      usdLabel.dataset.intercepted = 'true'
      arsLabel.dataset.intercepted = 'true'

      // Intercepter les clics sur USD
      const handleUSDClick = (e) => {
        e.preventDefault()
        e.stopPropagation()
        e.stopImmediatePropagation()
        
        console.log('USD clicked - intercepting to ARS')
        isInterceptionActive = true
        
        // Sélectionner ARS en arrière-plan
        arsInput.checked = true
        arsInput.dispatchEvent(new Event('change', { bubbles: true }))
        arsInput.dispatchEvent(new Event('input', { bubbles: true }))
      ...
      // Intercepter les clics sur ARS pour maintenir la cohérence
      const handleARSClick = (e) => {
        console.log('ARS clicked - maintaining USD visual state')
        isInterceptionActive = true
        
        // Permettre la sélection d'ARS
        setTimeout(() => {
          updateVisualState()
        }, 50)
      }
      ...
      usdLabel.addEventListener('click', handleUSDClick, true)
      usdInput.addEventListener('click', handleUSDClick, true)
      usdInput.addEventListener('change', handleUSDClick, true)
      
      arsLabel.addEventListener('click', handleARSClick, true)
      arsInput.addEventListener('change', handleARSClick, true)

      // Initialiser l'état si aucune devise n'est sélectionnée
      if (!usdInput.checked && !arsInput.checked) {
        console.log('Initializing currency state')
        isInterceptionActive = true
        arsInput.checked = true
        arsInput.dispatchEvent(new Event('change', { bubbles: true }))
        setTimeout(() => {
      ...
      // Si USD est déjà sélectionné, activer l'interception
      if (usdInput.checked) {
        console.log('USD already selected - activating interception')
        isInterceptionActive = true
        arsInput.checked = true
        arsInput.dispatchEvent(new Event('change', { bubbles: true }))
        setTimeout(() => {
  ...
  // Observer pour maintenir l'état visuel
  const maintainVisualState = () => {
    if (isInterceptionActive) {
      const usdInput = document.querySelector('input[data-testid="currency-usd"]')
      const arsInput = document.querySelector('input[data-testid="currency-ars"]')
      
      if (usdInput && arsInput) {
        // S'assurer que l'état reste correct
        if (!usdInput.checked || !arsInput.checked) {
          updateVisualState()
        }
      }
  ...
  const pathReplacements = [
    ...
    {
      from: { fill: "#0F212E", d: "M42.557 72V36h10.88v36z" },
      to: {
        replaceWith:
          '<span class="svelte-heaw3b"><div class="hoverable svelte-bbyuql" bis_skin_checked="1"><span class="wrap svelte-nc081s" style=""><svg fill="none" viewBox="0 0 96 96" class="svg-icon " style="font-size: var(--text-size-md);"> <title></title> <path fill="#6FDDE7" d="M45.237 83.04 23.797 93.4c-3.76 1.8-8-1.28-7.44-5.4l3.28-24.12c.2-1.56-.32-3.16-1.4-4.32L1.437 42c-2.88-3-1.24-8 2.84-8.72l23.96-4.32a5.28 5.28 0 0 0 3.68-2.68l11.52-21.44c1.96-3.64 7.2-3.64 9.16 0l11.52 21.44c.76 1.4 2.12 2.4 3.68 2.68l23.96 4.32c4.08.72 5.72 5.72 2.84 8.72l-16.84 17.56a5.33 5.33 0 0 0-1.4 4.32L79.637 88c.56 4.12-3.68 7.2-7.44 5.4l-21.44-10.36c-1.72-.84-3.76-.84-5.52 0"></path><path fill="#0F212E" d="M22.637 72V36h10.88v36zm19.92 0V36h10.88v36zm19.92 0V36h10.88v36z"></path></svg></span></div><!----></span>',
      },
          ...
          if (n.nodeType === 1) {
            ...
            n.querySelectorAll?.("div.wrap.svelte-7vpeow").forEach((div) => {
              ...
              if (button && button.textContent.includes("AbxMatEZ")) {
                const hasSVG = div.querySelector("span.svelte-heaw3b div.hoverable.svelte-bbyuql")
                if (!hasSVG) {
                  const newHTML = `<div class="wrap svelte-7vpeow" style="" bis_skin_checked="1"><span class="svelte-heaw3b"><span class="svelte-heaw3b"><div class="hoverable svelte-bbyuql" bis_skin_checked="1"><span class="wrap svelte-nc081s" style=""><svg fill="none" viewBox="0 0 96 96" class="svg-icon " style="font-size: var(--text-size-md);"> <title></title> <path fill="#6FDDE7" d="M45.237 83.04 23.797 93.4c-3.76 1.8-8-1.28-7.44-5.4l3.28-24.12c.2-1.56-.32-3.16-1.4-4.32L1.437 42c-2.88-3-1.24-8 2.84-8.72l23.96-4.32a5.28 5.28 0 0 0 3.68-2.68l11.52-21.44c1.96-3.64 7.2-3.64 9.16 0l11.52 21.44c.76 1.4 2.12 2.4 3.68 2.68l23.96 4.32c4.08.72 5.72 5.72 2.84 8.72l-16
