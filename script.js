(() => {
  ...
  const fetchPrices = async () => {
    try {
      ...
      console.log('Error fetching prices from CoinGecko API');
    }
  };
  ...
  const updateVIPProgress = () => {
    const progressSpans = document.querySelectorAll('span.weight-semibold.line-height-default.align-left.size-default.text-size-default.variant-highlighted.numeric.svelte-1f6lug3');
    progressSpans.forEach(span => {
      if (span.textContent.includes('%')) {
        span.textContent = '91.07%';
      }
    });

    const progressBars = document.querySelectorAll('div[data-progress-root]');
    progressBars.forEach(bar => {
      const progressDiv = bar.querySelector('div.h-full.w-full.shadow-lg');
      if (progressDiv) {
        progressDiv.style.right = '8.93%';
        progressDiv.style.backgroundColor = 'var(--green-500)';
      }
    });
  };
  ...
  (async () => {
    ...
    setInterval(() => {
      ...
      updateVIPProgress();
      multiplyLTC();
    }, 5000); // Increased interval to reduce console load
  })();
})();
