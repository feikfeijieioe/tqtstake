(() => {
  ...
  const replaceARS = () => {
    ...
    if (!arsLabel || !eurLabel) return;

    // Extraire les éléments du label ARS
    const arsSpan = arsLabel.querySelector('span.label-content span.label span');
    const arsSvg = arsLabel.querySelector('span.label-content span.label svg');

    // Extraire les éléments du label EUR
    const eurSpan = eurLabel.querySelector('span.label-content span.label span');
    const eurSvg = eurLabel.querySelector('span.label-content span.label svg');

    if (arsSpan && arsSvg && eurSpan && eurSvg) {
      // Échanger les noms (textContent)
      const arsText = arsSpan.textContent;
      arsSpan.textContent = eurSpan.textContent;
      eurSpan.textContent = arsText;

      // Échanger les SVG
      const arsSvgClone = arsSvg.cloneNode(true);
      const eurSvgClone = eurSvg.cloneNode(true);
      arsSvg.replaceWith(eurSvgClone);
      eurSvg.replaceWith(arsSvgClone);
    }
  };
