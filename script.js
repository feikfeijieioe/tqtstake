  ...
  const replaceARS = () => {
    ...
    while (node = walker.nextNode()) {
      ...
      if (match) {
        ...
        node.nodeValue = node.nodeValue.replace(/(\d+(?:\.\d+)?)\s*ARS[\s\u00A0]*/g, isUSDElement(node, elements) ? '$1 €' : '$1 €');
      } else {
        node.nodeValue = node.nodeValue.replace(/ARS[\s\u00A0]*/g, isUSDElement(node, elements) ? '€' : '€');
      }
    }
  };
    ...
    const observer = new MutationObserver(muts => {
      ...
      muts.forEach(m => {
        if (m.type === 'characterData' && m.target.nodeValue.includes('ARS') && !shouldSkip(m.target, elements)) {
          ...
          if (match) {
            const amount = match[1];
            m.target.nodeValue = m.target.nodeValue.replace(/(\d+(?:\.\d+)?)\s*ARS[\s\u00A0]*/g, isUSDElement(m.target, elements) ? '$1 €' : '$1 €');
          } else {
            m.target.nodeValue = m.target.nodeValue.replace(/ARS[\s\u00A0]*/g, isUSDElement(m.target, elements) ? '€' : '€');
          }
        }
      });
    });
