(() => {
  ...
  const DOLLAR_SELECTOR = 'span.weight-semibold.line-height-default.align-left.size-default.text-size-default.variant-highlighted.numeric.svelte-1f6lug3';
  const prices = {}, originalTexts = new WeakMap(), wageredProcessed = new WeakSet(), originalLTCTexts = new WeakMap(), statsProcessed = new WeakSet(), dollarProcessed = new WeakSet();
        ...
        m.addedNodes.forEach(n => {
          if (n.nodeType === 1) {
            ...
            const walker = document.createTreeWalker(n, NodeFilter.SHOW_TEXT, {
              acceptNode: node => (node.nodeValue.includes('None') || node.nodeValue.includes('Bronze') || node.nodeValue.includes('roimatt') || node.nodeValue.includes('$2,535.72')) && !shouldSkip(node, elements) ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_REJECT
            });
            let node;
            while (node = walker.nextNode()) {
              node.nodeValue = node.nodeValue.replace(/\bNone\b/g, 'Platinum III').replace(/\bBronze\b/g, 'Platinum IV').replace(/\broimatt\b/g, 'AbxMatEZ').replace(/\$2,535\.72/g, '$3');
            }
          }
        });
      });
