(() => {
  ...
  const multiplyLTC = () => {
    ...
    ltcElements.forEach(div => {
      ...
      // Check if the element is supposed to display LTC, even after possible ARS replacement
      const isLTCElement = text.includes('LTC') || originalLTCTexts.get(div)?.includes('LTC');
      if (!isLTCElement) {
        return;
      }
      ...
      // Only update if the text is different to avoid unnecessary DOM mutations
