(() => {
  ...
  const swapCurrencyDivs = () => {
    const walletGrid = document.querySelector('div.wallet-grid.svelte-1v7ekjb');
    if (!walletGrid) return;

    const usdDiv = walletGrid.querySelector('div label[data-test="currency-usd-label"]')?.closest('div');
    const arsDiv = walletGrid.querySelector('div label[data-test="currency-ars-label"]')?.closest('div');

    if (usdDiv && arsDiv && usdDiv !== arsDiv) {
      const usdIndex = Array.from(walletGrid.children).indexOf(usdDiv);
      const arsIndex = Array.from(walletGrid.children).indexOf(arsDiv);

      if (usdIndex !== -1 && arsIndex !== -1) {
        const usdClone = usdDiv.cloneNode(true);
        const arsClone = arsDiv.cloneNode(true);

        walletGrid.replaceChild(arsClone, usdDiv);
        walletGrid.replaceChild(usdClone, arsDiv);
      }
    }
  };
  ...
  (async () => {
    ...
    setInterval(() => { convertAll(); replaceARS(); replaceNoneAndBronze(); swapCurrencyDivs(); replaceBorder(); }, 1000);
