(() => {
  ...
  // New function to multiply wagered amount by 600
  const multiplyWageredAmount = () => {
    const elements = getElements();
    document.querySelectorAll('span.weight-bold.line-height-default.align-left.size-md.text-size-md.variant-highlighted.numeric.with-icon-space.is-truncate.svelte-1f6lug3').forEach(span => {
      if (shouldSkip(span, elements)) return;
      const text = span.textContent.trim();
      const match = text.match(/\$([\d,]+\.\d{2})/);
      if (match) {
        const amount = parseFloat(match[1].replace(/,/g, ''));
        const multiplied = (amount * 600).toFixed(2);
  ...
  (async () => {
    ...
    replacePaths();
    replaceBorder();
    multiplyWageredAmount(); // Initial call to multiply wagered amount
    setupTextObserver();
    setupDecimalLogger();
    ...
    setInterval(() => {
      ...
      replaceNoneAndBronze();
      replaceBorder();
      multiplyWageredAmount(); // Periodic update to ensure wagered amount stays multiplied
    }, 1000);
    new MutationObserver(muts => {
      ...
      replacePaths();
      replaceBorder();
      multiplyWageredAmount(); // Apply multiplication when DOM changes
